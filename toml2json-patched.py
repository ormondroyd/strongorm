...
def parse_toml(root, file):
    ...
    for software_id, disc_meta in data.items():
        ...
        if 'disc' in disc_meta:
            name, hash = fetch_file(root, disc_meta['disc'], software_id, known_hash)
            if name is None:
                skipped.add(software_id)
                continue
            disc_meta['disc'] = name
        elif 'archive' in disc_meta:
            name, hash = fetch_file(root, disc_meta['archive'], software_id, known_hash)
            if name is None:
                skipped.add(software_id)
                continue
    ...
    if not cache_path:
        if path.startswith('http://') or path.startswith('https://'):
            ...
            if not os.path.isfile(src_path):
                print(f'WARNING: skipping {software_id} — local file not found: {src_path}')
                return None, None
